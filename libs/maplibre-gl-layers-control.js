...
`;
  ...
  class LayersControl {
    ...
    getBaseLayerIds() {
      return this.map.getStyle().layers?.filter((l) => l.metadata?.isbaselayer).map((l) => l.id) ?? [];
    }
    buildPanel() {
      this.panel.innerHTML = `<h2 class='panel-title'>${this.title}</h2>`;
      const labelledIds = Object.keys(this.customLabels);
      const layers = (this.map.getStyle().layers ?? []).filter(
        (l) => !labelledIds.length || labelledIds.includes(l.id)
      );
    ...
    radioButtonControlAdd(layerId, labelTitle, parent) {
    ...
    checkBoxControlAdd(layerId, labelTitle, parent) {
      const input = document.createElement("input");
      input.type = "checkbox";
      input.id = layerId;
      const vis = this.map.getLayoutProperty(layerId, "visibility") !== "none";
      input.checked = vis;
      this.map.setLayoutProperty(layerId, "visibility", vis ? "visible" : "none");
      input.addEventListener("change", (e) => {
        const chk = e.target.checked;
        const visibility = chk ? "visible" : "none";
    ...
    rangeControlAdd(layerId, type, parent) {
    ...
    async fetchAndRenderLegend(layerId, parent) {
